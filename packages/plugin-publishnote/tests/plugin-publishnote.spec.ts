...
import pluginPublishnotePlugin from "../dist/index.js";
import {
  createExpressApp,
...
describe("@dkg/plugin-publishnote checks", function () {
  ...
  beforeEach(async () => {
    ...
    // Initialize plugin
    pluginPublishnotePlugin(mockDkgContext, mockMcpServer, apiRouter);
    await connect();
    app.use("/", apiRouter);
  });
  ...
  describe("Plugin Configuration", () => {
    it("should create plugin without errors", () => {
      expect(pluginPublishnotePlugin).to.be.a("function");
    });
  });
